...
*   **Backend / AI Engine:** Python 3, Flask, Google GenAI SDK (Dual-Model: Gemini 3.6 Flash + Gemini 3.1 Pro)
*   **Monetization & Analytics:** Stripe (Payments) & PostHog (Analytics, Funnels, A/B Testing)
...
### A. Ingestion & AI Synthesis (`engine/ingest.py`)
...
*   **Process:** 
    ...
    2. **Dual-Model Processing:** 
       - Uses **Gemini 3.6 Flash** to cluster and deduplicate large volumes of incoming articles (to save on input token costs).
       - Uses **Gemini 3.1 Pro** to synthesize the clustered arrays using a highly specific system prompt (`engine/rubric.md`) instructing it to adopt an irreverent, sarcastic, vigilant watchdog persona.
    3. Gemini evaluates the article's relevance (0-100 score). If `>= 65`, it formats a headline, framing lead, blockquote, kicker, category, and Tip CTA, enforcing output in strict JSON format.
...
*   **Process:**
    ...
    2. The Flask server (running as a `systemd` service) receives the payload, extracts the `draft_id` from the callback, and queries SQLite.
    ...
    4. **Image Sourcing:** The bot calls the **Brave Image Search API** (migrated from Unsplash) to fetch a relevant hero image. If the API fails or rate limits, the article proceeds gracefully with a fallback placeholder.
...
*   **Process:** 
    1. Queries `vanguard.db` for articles marked "published" that do not have a hero image attached.
    2. Attempts to fetch an image again via the Brave Image Search API. If successful, pulls the live Markdown file from GitHub, updates the frontmatter, and pushes a new commit to restore the image.

### D. Monetization & Analytics (VAME)
*   **PostHog Setup:** The frontend is instrumented with PostHog in `src/components/BaseHead.astro` for comprehensive pageview and session recording.
*   **Custom Event Tracking:** The `TipJar.astro` and `SidebarAffiliate.astro` components utilize HTML `data-posthog` attributes for smart autocapture.
*   **Conversion Funnels:** Custom Actions (e.g., `Sidebar Affiliate Click`) are defined in the PostHog dashboard to track the exact conversion rate from Article Pageview to Affiliate/Donation Click.
*   **Payments:** The Tip Jar component links directly to live Stripe payment URLs.

### E. Deployment (Cloudflare Pages)
...
**CRITICAL:** Environment variables for all scripts must be stored in the root directory (`/root/vanguard-wire/.env`) on the VPS to prevent synchronization issues. Avoid maintaining separate `.env` files in `engine/`.
...
*   `BRAVE_API_KEY` (For image sourcing)
